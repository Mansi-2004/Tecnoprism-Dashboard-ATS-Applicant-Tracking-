...
export default function Login({ initialIsLogin = true }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("ats-role", data.role || (isLogin ? "recruiter" : role));
  ...
  const handleSocialLogin = (provider) => {
    alert(`${provider} login is currently in demo mode. Please use the normal login/signup form instead.`);
  };
        ...
        <div className="w-full max-w-md space-y-8 relative z-10">
              ...
              <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-400">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <button
                    ...
                    className={`p-4 rounded-xl border-2 transition-all duration-300 ${role === "recruiter"
                      ? "border-primary bg-primary/5 shadow-lg shadow-primary/20"
                      : "border-muted-foreground/10 hover:border-primary/30 hover:bg-muted/30"
                      }`}
                  >
                    <User className={`h-6 w-6 mx-auto mb-2 ${role === "recruiter" ? "text-primary" : "text-muted-foreground"}`} />
                    <p className={`text-xs font-bold ${role === "recruiter" ? "text-primary" : "text-muted-foreground"}`}>Recruiter</p>
                  </button>
                </div>
              </div>
            )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <Button
              ...
              className="h-12 border-muted-foreground/10 rounded-2xl hover:bg-muted/50 hover:border-primary/20 transition-all flex items-center justify-center gap-3 group"
            >
