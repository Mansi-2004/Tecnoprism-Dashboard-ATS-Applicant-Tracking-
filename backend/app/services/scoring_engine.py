...
def get_embedding_model():
    ...
    if _embedding_model is None:
        print("Loading SentenceTransformer model (this may take a while)...")
...
class ResumeScorer:
    ...
    def score_application(
        self,
        parsed_applicant_data: Dict[str, Any],
        resume_text: str,
        job_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Score an application based on applicant data and job requirements.
        ...
        # Ensure parsed_applicant_data is valid dict
        if not parsed_applicant_data:
        ...
        skill_result = self._score_skills(
            parsed_applicant_data.get('skills', []),
            job_required_skills,
            resume_text
        )
        ...
        experience_score = self._score_experience(
            parsed_applicant_data.get('experience_years', 0),
            job_experience_years
        )
        
        # 3. EDUCATION SCORING
        education_score = self._score_education(
            parsed_applicant_data.get('education', []),
            job_education
        )
        ...
        return {
            ...
            "experience_match": self._clamp_score((
                100 if parsed_applicant_data.get('experience_years', 0) >= job_experience_years
                else (parsed_applicant_data.get('experience_years', 0) / max(job_experience_years, 1)) * 100
            )),
    ...
    def _score_skills(
        ...
        # Match applicant skills against job requirements
        matched_skills = []
        matched_weight = 0.0
        missing_skills = []
        ...
        for job_skill in weighted_job_skills:
            ...
            # Check if skill is in applicant skills or resume text
            skill_found = (
                skill_name in applicant_skills_lower or
    ...
    def _score_experience(
        ...
        If applicant_years >= required_years: 100
        Else: (applicant_years / required_years) * 100
        """
        if required_years == 0:
            return 100.0
        ...
        score = (applicant_years / required_years) * 100
        return score
    
    def _score_education(
        self,
        applicant_education: List[str],
        required_education: Optional[str]
    ) -> float:
        """
        Score education match.
        Check if applicant has relevant degree keywords.
        """
        ...
        applicant_lower = [e.lower() for e in applicant_education]
        ...
        max_score = 0.0
        for applicant_deg in applicant_lower:
            if 'phd' in required_lower and 'phd' in applicant_deg:
                max_score = 100.0
            elif 'master' in required_lower and ('master' in applicant_deg or 'msc' in applicant_deg):
                max_score = max(max_score, 90.0)
            elif 'bachelor' in required_lower and ('bachelor' in applicant_deg or 'bsc' in applicant_deg or 'bs' in applicant_deg):
                max_score = max(max_score, 80.0)
            elif applicant_deg in required_lower or required_lower in applicant_deg:
    ...
    def _score_semantic(
        ...
        try:
            ...
            # 1. Skills similarity
            applicant_skills_text = " ".join(parsed_applicant_data.get('skills', []))
            job_skills_text = " ".join(
                [s.get('name', s) if isinstance(s, dict) else s
                 for s in job_data.get('weighted_skills', []) or job_data.get('required_skills', [])]
            )
            
            if applicant_skills_text and job_skills_text:
                skill_sim = self._compute_semantic_similarity(applicant_skills_text, job_skills_text)
                similarities.append(skill_sim)
            
            # 2. Experience similarity
            applicant_experience_text = resume_text[
            ...
            job_description = job_data.get('description', '')
            if applicant_experience_text and job_description:
                exp_sim = self._compute_semantic_similarity(applicant_experience_text, job_description)
...
async def evaluate_application_v2(
    ...
    """
    Evaluate an applicant application using the production-grade scorer.
    
    Returns scoring breakdown with all metrics.
    """
    scorer = ResumeScorer()
    return scorer.score_application(parsed_applicant_data, resume_text, job_data)
