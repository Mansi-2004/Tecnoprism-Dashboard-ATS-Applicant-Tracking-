...
async def register(user_in: UserCreate, db: AsyncIOMotorDatabase = Depends(get_db)):
    ...
    return {"access_token": access_token, "token_type": "bearer", "role": user_in.role.value, "name": user_in.name, "profile_photo": user_doc.get("profile_photo")}
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncIOMotorDatabase = Depends(get_db)):
    if form_data.username == settings.ADMIN_EMAIL:
        ...
        return {"access_token": access_token, "token_type": "bearer", "role": UserRole.ADMIN.value, "name": "Admin"}
    ...
    return {"access_token": access_token, "token_type": "bearer", "role": user["role"], "name": user.get("name", user["email"].split("@")[0]), "profile_photo": user.get("profile_photo")}
