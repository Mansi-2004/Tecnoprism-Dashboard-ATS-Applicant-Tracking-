...
async def get_current_user(token: str = Depends(oauth2_scheme), db: AsyncIOMotorDatabase = Depends(get_db)) -> UserInDB:
    ...
    print(f"Auth Attempt - Email: {token_data.email}, Role: {token_data.role}")
    
    if token_data.email == settings.ADMIN_EMAIL and token_data.role == UserRole.ADMIN.value:
        print("Authenticated as Static Admin")
        return UserInDB(
            **{
                "_id": "admin-static",
    ...
    if user is None:
        print(f"User not found in DB for email: {token_data.email}")
        raise credentials_exception
    
    print(f"Authenticated as DB User: {user.get('name')} (Role: {user.get('role')})")
